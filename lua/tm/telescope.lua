...
-- TmFind: поиск в текущем пространстве
function M.find_notes(search_text, space_id)
  if not search_text or search_text == "" then
    vim.notify("Необходимо указать текст для поиска", vim.log.levels.WARN)
    return
  end

  core.find_notes(search_text, space_id, false, function(result)
    if not result.success then
      vim.notify("Ошибка поиска", vim.log.levels.ERROR)
      return
    end

    if #result.results == 0 then
      vim.notify("Вхождений не найдено", vim.log.levels.INFO)
      return
    end
    ...
    local refresh_fn = function()
      M.find_notes(search_text, space_id)
    end
    ...
    pickers
      .new(opts, {
        prompt_title = "Поиск: " .. search_text,
        finder = finders.new_table({
          results = result.results,
          entry_maker = function(item)
            return {
              value = item.context,
              display = "[" .. item.id .. "]: " .. truncate_text(item.context),
              ordinal = item.context,
              note_id = item.id,
              full_text = item.context,
            }
          end,
        }),
        sorter = conf.generic_sorter(opts),
...
-- TmFindGlobal: глобальный поиск
function M.find_global(search_text)
  if not search_text or search_text == "" then
    vim.notify("Необходимо указать текст для поиска", vim.log.levels.WARN)
    return
  end

  core.find_notes(search_text, nil, true, function(result)
    if not result.success then
      vim.notify("Ошибка поиска", vim.log.levels.ERROR)
      return
    end
    ...
    pickers
      .new(opts, {
        prompt_title = "Глобальный поиск: " .. search_text,
        finder = finders.new_table({
          results = result.results,
          entry_maker = function(item)
            return {
        ...
        sorter = conf.generic_sorter(opts),
        attach_mappings = function(prompt_bufnr, map)
          -- Для глобального поиска нужно определить workspace_id для каждой заметки
          -- Это упрощенная версия, в реальности нужно парсить вывод tm find -g
          local delete_note = function()
            local selection = action_state.get_selected_entry()
            vim.notify(
              "Удаление из глобального поиска требует указания пространства",
              vim.log.levels.WARN
            )
          end
